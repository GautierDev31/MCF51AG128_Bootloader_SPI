# 03/05 Flasher SPI avec protocole de communication S19 adapté communicant en réseau avec la FEB
...
# Maj V3
# Changement du protocole de communication et des commandes reseau

# Maj V2.2
# Modification de l'interface graphique
# Possibilité d'envoyer une seule trame qui respecte le protocole de communication
# Gestion de la reception des codes status de la version bootloader V2.2

# Maj V2.1 
# Ajout threads avec IHM et check status
...
class Send_network (QThread, threading.Thread):
    ...
    def run(self):
        ...
        th2 = threading.Thread(target=self.Trame_reception, args=("WRITING",))
        th1.start()
        th2.start()
        th1.join()
        th2.join()
    ...
    def Run_send_and_receive_one_word(self, input):
        ...
        th2 = threading.Thread(target=self.Trame_reception, args=("SIMPLE_RESPONSE",))
        th1.start()
        th2.start()
        th1.join()
        th2.join()
        ...
        if type == "WRITING" :
            while a != 0:   
                ...
                if (i == 3 ) :
                    ...
                    #print("status : " + str(int(MESSAGE_STR, 16)))
                    print("status Ecriture Flash : " + CODE_STATUS)
                elif(i == 1) :
                    print("status Erase Flash : " + CODE_STATUS)
                MESSAGE_STR = "0x"
    ...
    def Flash_write_from_file(self) :
        ...
        while flag != 1 :
            ...
            if (f.read(2) == "S7") :
                ...
                self.Send_one_word(310, 1)  #Demande status
                self.Send_one_word(310, 0)  #Lecture status
                ...
                self.Send_one_word(310, 1)  #Demande status
                self.Send_one_word(310, 0)  #Lecture status
                self.Send_one_word(330, 1) #Demande checksum
                self.Send_one_word(330, 0) #Lecture checksum

                self.Wait_until_trame_reception(1) #En attente de la reception de la checksum
    ...
    def Flash_write_one_trame(self, address, memory):
        ...
        self.Send_one_word (310, 1) #Demand status
        self.Send_one_word (310, 0) #Check status
        self.Send_one_word (memory_msb, 1) #memory msb
        self.Send_one_word (memory_lsb, 1) #memory msb
        self.Send_one_word (310, 1) #Demand status
        self.Send_one_word (310, 0) #Check status
        self.Send_one_word (330, 1) #Demand checksum
        self.Send_one_word (330, 0) #Checksum
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        #Texte
        self.pushButton_Stop.setText("Stop transfert")
    ...
    def on_click_check_status(self):
        self.net.Run_send_and_receive_one_word(310)
    ...
    def on_click_Run_send_one_trame(self):
        ...
        self.net.Run_send_one_trame(address, memory)
