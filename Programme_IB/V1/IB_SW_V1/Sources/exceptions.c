/*************************************************************************************************************************************/
/* file : exception.c / Rev : 1.2 / Author : Alexis LÉVÊQUE / Date : 11/03/2016														 */
/*************************************************************************************************************************************/
/* History :																														 */
/*																																	 */
/* Rev : 1.2 :	remove the test of the transmit buffer empty flag on the SPI IT (read request)								 		 */
/*************************************************************************************************************************************/
/* Rev : 1.1 :	use the bit structure on the ITSPI2 function																 		 */
/*************************************************************************************************************************************/
/* Rev : 1.0 :	creation based on the generic file to IB V1 (IB_SW_V1)																 */
/*				_ IT 10ms																											 */
/*				_ ITIIC																												 */
/*				_ ITADC																												 */
/*				_ ITSPI2																											 */
...
extern unsigned char it_10ms;
extern unsigned char it_i2c;
extern unsigned char tbl_send_DACI2C[16][4];
extern unsigned char ctr_send_DACI2C;
extern unsigned char ctr_send_numDACI2C;
extern unsigned char it_adc;
extern unsigned short tbl_convertbrut_ADC[24]; /* Table of ADC untreated conversions */
extern unsigned char ch_ADC; /* ADC channel number */
extern unsigned char it_spi2rx;
extern unsigned char ctr_spi2tx;
extern unsigned short tbl_spi2rx[3];
extern unsigned char tbl_spi2tx[320];
extern unsigned char ctr_spi2rx;

//extern unsigned short REF_HT_DU0_DAC; /* DEBUG !!!! */
...
__declspec(register_abi) void interrupt IT10ms(void)
{
//	SPARE_PTC5 = !SPARE_PTC5; /* Toggle output : VISU_DEBUG */

	it_10ms++;
	RTCSC_RTIF = 1;	/* Enable the next IT: clear RTCSC.RTIF */
}

__declspec(register_abi) void interrupt ITIIC(void)
{
//	SPARE_PTC3 = 1; /* Active output : VISU_DEBUG */
	
	if(IICS_IICIF)
	{
		IICS_IICIF = 1;
		if(ctr_send_DACI2C < 3)
		{
			ctr_send_DACI2C++;
		...
		{
			IICC1_MST = 0; /* Master mode disable */
			ctr_send_DACI2C = 0;
			ctr_send_numDACI2C++;
			it_i2c = 1;
		}
	}
//	SPARE_PTC3 = 0; /* Deactivate output : VISU_DEBUG */
	
}

__declspec(register_abi) void interrupt ITADC(void)
{
//	SPARE_PTC3 = 1; /* Active output : VISU_DEBUG */

	tbl_convertbrut_ADC[ch_ADC] = ADCRA;
	it_adc = 1;
...
{
//	SPARE_PTC5 = 1; /* Active output : VISU_DEBUG */

	if(SPI2S_SPRF) /* Rx */
	{
		spi2_rx_brut.U8[0] = SPI2DH;
		spi2_rx_brut.U8[1] = SPI2DL;
		tbl_spi2rx[ctr_spi2rx] = spi2_rx_brut.U16;
		if(ctr_spi2tx) /* Sending ? */
		{
			if(SPI2S_SPTEF) /* Tx register ready ? */
			{
				offset_tx.U16++;
				SPI2DH = tbl_spi2tx[offset_tx.U16];
				offset_tx.U16++;
				SPI2DL = tbl_spi2tx[offset_tx.U16];
				ctr_spi2tx = 0; /* Sending finished */
			}
		}
		if(!ctr_spi2rx) /* First word ? */
		{
			if(spi2_rx_brut.bits.bit15) /* Read ? */
			{
				offset_tx.U16 = spi2_rx_brut.U16;
				offset_tx.bits.bit15 = 0; /* Offset calculation */
				SPI2DH = tbl_spi2tx[offset_tx.U16];
				offset_tx.U16++;
				SPI2DL = tbl_spi2tx[offset_tx.U16];
				ctr_spi2tx++; /* Sending in progress */
			}
			else
			{
				SPI2DH = 0;
...
//	SPARE_PTC5 = 0; /* Deactivate output : VISU_DEBUG */
}
...
// __declspec(weak) vectorTableEntryType vector_90  @Vadc = asm_exception_handler;
 __declspec(weak) vectorTableEntryType vector_90  @Vadc = &ITADC;
...
// __declspec(weak) vectorTableEntryType vector_99  @Viic = asm_exception_handler;
 __declspec(weak) vectorTableEntryType vector_99  @Viic = &ITIIC;
...
// __declspec(weak) vectorTableEntryType vector_113 @Vrtc_wdg = asm_exception_handler;
 __declspec(weak) vectorTableEntryType vector_113 @Vrtc_wdg = &IT10ms;
