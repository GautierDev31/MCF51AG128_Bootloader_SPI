# 07/07 Flasher SPI avec protocole de communication S19 adaptÃ© communicant en rÃ©seau avec la FEB
...
class Network_communication (QThread, threading.Thread):
    ...
    #Variables partagÃ©s avec la classe APP
    ...
    def __init__(self):
        ...
        self.i_data = 0 #Variable qui s'incrÃ©mente Ã  chaque Ã©criture d'un mot de 16 bits dans la memoire


    def Run_send_and_receive_one_word(self, input):
        """Lancement des thread de lecture et d'Ã©criture pour ecrire un seul mot.
        """
    ...
    def Trame_reception(self, thread, type) :
        """Ecoute du port de la FEB.
            ...
            A chaque reception la fonction change l'Ã©tat de reception_state Ã  "RECEIVED"
        """
    ...
    def Send_one_word(self, input, type):
        """Envoie du message en reseau
            ...
            Message a envoyer.
            type : int
            Definis la commande d'Ã©criture ou de lecture. 1 : ecriture, 0 : lecture

            Envoie le message en input aprÃ¨s construction de la trame.
        """
        UDP_IP = "192.168.1.21"
        UDP_PORT = 1234
        time.sleep(0.005)
    ...
    def chcksum(self,value) :
        """Calcul de la checksum.
            ...
            Prend en entrÃ©e la chaine de caractÃ¨re comprenant la taille, l'addresse et la memoire.
            On convertie la chaine de caractÃ¨re en binaire et on fait la somme de tout les bits Ã  1.
        """
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        self.label_separator.setGeometry(QtCore.QRect(20, 230, 211, 31))
        self.label_separator.setObjectName("label")
        self.label_separator.setText("--------------------------")
        #Check status
        self.pushButton_CheckStatus = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_CheckStatus.setGeometry(QtCore.QRect(20, 280, 111, 31))
        ...
        self.pushButton_Reset_micro.setGeometry(QtCore.QRect(20, 330, 211, 31))
    ...
    def openFileNameDialog(self):
        ...
        Selection du fichier S19 utilisÃ© pour reflasher le microcontroleur
        """
        global filename
    ...
    def Flash_write_from_file(self) :
        """Envoie du fichier S19 pour reflasher le microcontroleur
            ...
            Envoie les donnÃ©es du S19 en respectant le protocole de communication
            La fonction fonctionne en parrallÃ¨le de la fonction Trame_reception pour lire les reponses du microcontroleur.

            La checksum calculÃ© pour chaque ligne d'envoie est a somme des bytes a 1 :
            - Du nombre de mots de 32 bits envoyÃ©s
            - De l'adresse envoyÃ©
            - Des mots de la mÃ©moire envoyÃ©
            Le calcul de la checksum cotÃ© client est le mÃªme que cotÃ© microcontroleur.
            AprÃ¨s l'envoie d'une ligne, la checksum calculÃ© par le microcontroleur est demandÃ© et comparÃ© avec celle calculÃ© cotÃ© client 
            pour savoir si le microcontroleur a correctement reÃ§u toutes les donnÃ©es.
        """


        # Check de la validitÃ© du fichier S19 selectionnÃ©
        f = open(self.net.fileName, "r")
        if (f.read(2) != "S0") :
            f.read(9)
            if(f.read(2) != "S3") :
                print("Le fichier selectionnÃ© n'est pas un fichier S19 ou n'est pas compatible avec le microcontroleur")
                return
        f.close()
        ...
        if (self.net.response_spi_received != 30) :
            if (self.net.response_spi_received != 10):
                if (self.net.response_spi_received != 7):
                    print("Impossible to communicate with the uC")
                    return
        print("Status Ready")

        # Effacement de la mÃ©moire flash de l'addresse 0x2000 Ã  0x1F800
        print("----- Erase Flash -----") 
        ...
        # Check de l'Ã©tat du microcontroleur aprÃ¨s l'effacement de la mÃ©moire
        print("----- Check Status -----") 
        self.net.Run_send_and_receive_one_word(410)
        self.net.Run_send_and_receive_one_word(410)
        self.net.Wait_until_trame_reception(1)
        ...
        # Ecriture de la mÃ©moire flash a partir du fichier S19
        self.text_Action.setText("(2/2) Programming Flash...")        
        self.net.i_data = 0
        self.progress.setValue(self.net.i_data)
        self.progress.setMaximum(self.net.nb_memory)
        while flag != 1 :
            #L'envoie ne s'arrÃªte que lorsque la derniÃ¨re ligne commence par "S7"
            if (f.read(2) == "S7") :
                f.read(2)
                address = "00001FF0"
                address_msb = int(address[0:4], 16)
                address_lsb = int(address[4:8], 16)
                memory_msb = int(f.read(4), 16) 
                memory_lsb = int(f.read(4), 16)
                self.net.Send_one_word (300, 1) #CMD Write
                self.net.Send_one_word (1, 1) #CMD Write
                self.net.Send_one_word(address_msb, 1) #Envoie de l'adresse msb
                self.net.Send_one_word(address_lsb, 1) #Envoie de l'adresse lsb
                self.net.Send_one_word(memory_msb, 1) #Envoie de l'adresse msb
                self.net.Send_one_word(memory_lsb, 1) #Envoie de l'adresse lsb
                self.net.Send_one_word (310, 1) #CMD Write
                self.text_Action.setText("successful")
                ...
                self.net.Send_one_word (300, 1) #Envoie de la commande d'Ã©criture
    ...
    def on_click_send_SPI(self):
        """ Cette action de click permet de lancer le reflash du microcontroleur si un fichier S19 a bien Ã©tÃ© selectionnÃ©
        """
